...
def Update_field_Customer(request,first_name,last_name,email,phone_number,Address,City,state,postal_Code):
    a = serch_field_Customer(request, first_name)
...
def serch_field_Products(request, Product_name):
    try:
        word = Products.objects.get(Product_name=Product_name)
        word_dict = {'Product_name': word.Product_name, 'description': word.description,'price':word.price,'image': word.image,'category id':word.image}
        return JsonResponse(word_dict)
